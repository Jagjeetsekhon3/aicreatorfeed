...
const TYPE_CONFIG: Record<string, { icon: string; color: string; label: (actor: string, msg?: string) => string }> = {
  ...
  comment: { icon: '💬', color: '#FF6D1F', label: (a, m) => `${a} commented: "${m}"` },
  reply:   { icon: '↩',  color: '#FF6D1F', label: (a, m) => `${a} replied: "${m}"` },
  ...
  return (
    <div style={{ maxWidth: '640px', margin: '0 auto', padding: '24px 0 80px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: 900, color: '#FAF3E1', margin: 0 }}>Notifications</h1>
          {unreadCount > 0 && <p style={{ fontSize: '13px', color: '#9a8f7a', marginTop: '3px' }}>{unreadCount} unread</p>}
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          {unreadCount > 0 && (
            <button onClick={markAllRead} disabled={marking} style={{ fontSize: '12px', fontWeight: 600, padding: '7px 14px', borderRadius: '8px', border: 'none', background: 'rgba(255,109,31,0.1)', color: '#FF6D1F', cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.07)', marginBottom: '16px' }}>
        {[
          ...
          <button key={key} onClick={() => setFilter(key as any)} style={{
            ...
            borderBottom: `2px solid ${filter === key ? '#FF6D1F' : 'transparent'}`,
            marginBottom: '-1px',
          }}>{label}</button>
        ))}
      </div>
            ...
            >
              ...
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <Link href={`/profile/${actor?.username}`} onClick={e => e.stopPropagation()} style={{ textDecoration: 'none' }}>
                  {actor?.avatar_url
                    ...
                    : <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,109,31,0.2)', color: '#FF6D1F', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '17px', fontWeight: 700 }}>{actor?.full_name?.[0] || '?'}</div>
                  }
                </Link>
                {/* Type icon badge */}
                <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
                  <div>
                    <span style={{ fontSize: '14px', color: '#FAF3E1', lineHeight: 1.5 }}>
                      <Link href={`/profile/${actor?.username}`} onClick={e => e.stopPropagation()} style={{ fontWeight: 700, color: '#FAF3E1', textDecoration: 'none' }}>
                        {actor?.full_name}
                      </Link>
                      ...
                      {' '}
                      <span style={{ color: '#F5E7C6', fontWeight: 400 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                    ...
                    {/* Unread dot */}
                    {!notif.read && <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#FF6D1F', flexShrink: 0 }} />}
                  </div>
                </div>
              </div>
            </div>
