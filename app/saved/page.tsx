...
export default function SavedPage() {
  ...
  if (authed === false) return (
    <div style={{ minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px', textAlign: 'center' }}>
      ...
      <h2 style={{ fontSize: '20px', fontWeight: 700, color: 'var(--color-cream)', margin: 0 }}>Sign in to see saved posts</h2>
      <p style={{ color: '#9a8f7a', fontSize: '14px', margin: 0 }}>Bookmark posts to save them for later.</p>
      <Link href="/auth/login" style={{ background: 'var(--color-primary)', color: '#fff', fontWeight: 700, padding: '10px 24px', borderRadius: '10px', textDecoration: 'none', fontSize: '14px' }}>
        Sign in
      </Link>
    </div>
  )
  ...
  return (
    <div style={{ maxWidth: '680px', margin: '0 auto', padding: '24px 0 80px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: 900, color: 'var(--color-cream)', margin: 0 }}>Saved posts</h1>
      ...
      {!loading && posts.length === 0 && (
        <div style={{ textAlign: 'center', padding: '80px 20px', animation: 'fadeIn 0.3s ease' }}>
          ...
          <h3 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--color-cream)', marginBottom: '8px' }}>No saved posts yet</h3>
          <p style={{ color: '#9a8f7a', fontSize: '14px', marginBottom: '24px', lineHeight: 1.6 }}>
            Tap the bookmark icon on any post to save it here.
          </p>
          <Link href="/explore" style={{ display: 'inline-block', background: 'var(--color-primary)', color: '#fff', fontWeight: 700, padding: '10px 24px', borderRadius: '10px', textDecoration: 'none', fontSize: '14px' }}>
            Explore posts
          </Link>
        </div>
      )}
          ...
          {hasMore && (
            <div style={{ textAlign: 'center', paddingTop: '8px' }}>
              <button
                ...
                style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--color-beige)', padding: '10px 28px', borderRadius: '10px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
              >
