...
export default function VerifyPage() {
  ...
  if (profile?.paid_verified || profile?.is_verified) return (
    <div style={{ maxWidth: '480px', margin: '60px auto', padding: '0 16px', textAlign: 'center' }}>
      ...
      <h1 style={{ fontSize: '26px', fontWeight: 900, color: 'var(--color-cream)', marginBottom: '10px' }}>Already verified!</h1>
      ...
      {profile.subscription_expires_at && (
        <p style={{ color: '#9a8f7a', fontSize: '13px', marginBottom: '28px' }}>
          Subscription active until: <strong style={{ color: 'var(--color-cream)' }}>{new Date(profile.subscription_expires_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}</strong>
        </p>
      )}
      <Link href={`/profile/${profile.username}`} style={{ display: 'inline-block', background: 'var(--color-primary)', color: '#fff', fontWeight: 700, padding: '12px 28px', borderRadius: '12px', textDecoration: 'none' }}>
        View your profile →
      </Link>
    </div>
  )
  ...
  if (success) return (
    <div style={{ maxWidth: '480px', margin: '60px auto', padding: '0 16px', textAlign: 'center' }}>
      <div style={{ fontSize: '56px', marginBottom: '16px' }}>🎉</div>
      <h1 style={{ fontSize: '26px', fontWeight: 900, color: 'var(--color-primary)', marginBottom: '10px' }}>You're verified!</h1>
  ...
  return (
    <div style={{ maxWidth: '560px', margin: '40px auto', padding: '0 16px 80px' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '36px', animation: 'fadeIn 0.3s ease' }}>
        ...
        <h1 style={{ fontSize: '28px', fontWeight: 900, color: 'var(--color-cream)', marginBottom: '8px' }}>Get Verified</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '24px', animation: 'fadeIn 0.35s ease' }}>
        {PLANS.map(plan => (
          <div key={plan.id} onClick={() => setSelected(plan.id)}
            ...
            {plan.popular && (
              <div style={{ position: 'absolute', top: '-10px', left: '50%', transform: 'translateX(-50%)', background: 'var(--color-primary)', color: '#fff', fontSize: '10px', fontWeight: 800, padding: '3px 10px', borderRadius: '999px', whiteSpace: 'nowrap' }}>BEST VALUE</div>
            )}
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
              <div style={{ width: '20px', height: '20px', borderRadius: '50%', border: `2px solid ${selected === plan.id ? 'var(--color-primary)' : 'rgba(255,255,255,0.2)'}`, background: selected === plan.id ? 'var(--color-primary)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
